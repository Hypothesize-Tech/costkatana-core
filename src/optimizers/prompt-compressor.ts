...
export class PromptCompressor {
    ...
    async compressPrompt(
        ...
        // 3. Abbreviation
        const abbreviationCompression = await this.compressWithAbbreviations(prompt, targetModel, targetProvider);
        if (abbreviationCompression) suggestions.push(abbreviationCompression);

        // 4. Deduplication
        const deduplicationCompression = await this.deduplicateContent(prompt, targetModel, targetProvider);
    ...
    private async compressJSON(
        ...
        const compressedTokens = await TokenCounter.countTokens(compressedPrompt, targetProvider, targetModel);
    ...
    private async compressPatterns(
        ...
        for (const pattern of patterns) {
            ...
            if (pattern.length * occurrences > pattern.length + ref.length * occurrences + 50) {
                ...
                totalSaved += pattern.length * (occurrences - 1) - ref.length * occurrences - definition.length;
            }
        }
    ...
    private async deduplicateContent(
        ...
        const originalTokens = await TokenCounter.countTokens(prompt, targetProvider, targetModel);
        const deduplicatedTokens = await TokenCounter.countTokens(deduplicatedPrompt, targetProvider, targetModel);
        const savings = ((originalTokens - deduplicatedTokens) / originalTokens) * 100;

        if (savings < 5) return null;

        const totalDuplicates = Array.from(duplicates.values()).reduce((sum, count) => sum + Math.max(0, count - 1), 0);
