...
export class RequestFusion {
  ...
  async analyzeRequestsForFusion(requests: FusionRequest[]): Promise<OptimizationSuggestion[]> {
  ...
  private detectParallelPattern(requests: FusionRequest[]): boolean {
    ...
    const hasListRequests =
      prompts.filter(p => p.includes('list') || p.includes('enumerate') || p.includes('provide'))
        .length >=
      requests.length * 0.6;
  ...
  private async calculateTotalTokens(requests: FusionRequest[]): Promise<number> {
    ...
    for (const request of requests) {
      total += await TokenCounter.countTokens(request.prompt, request.provider, request.model);
    }
    return total;
  }
