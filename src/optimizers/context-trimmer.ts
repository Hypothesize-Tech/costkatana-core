...
export class ContextTrimmer {
    ...
    async trimContext(
        ...
        const currentTokens = await TokenCounter.countTokens(currentContext, targetProvider, targetModel);
    ...
    private async applySlidingWindow(
        ...
        const trimmedTokens = await TokenCounter.countTokens(trimmedContext, targetProvider, targetModel);
    ...
    private async applyRelevanceFiltering(
        ...
        const originalTokens = await TokenCounter.countTokens(originalContext, targetProvider, targetModel);
        const filteredTokens = await TokenCounter.countTokens(filteredContext, targetProvider, targetModel);
...
Provide a summary in 2-3 paragraphs that captures the essential information.`;
        ...
        try {
            ...
            const originalContext = messages.map(m => `${m.role}: ${m.content}`).join('\n');
            const originalTokens = await TokenCounter.countTokens(originalContext, targetProvider, targetModel);
            const summarizedTokens = await TokenCounter.countTokens(summarizedContext, targetProvider, targetModel);
    ...
    private async applyImportanceScoring(
        ...
        const scoredMessages = messages.map((msg, index) => {
            ...
            const importantKeywords = ['important', 'critical', 'must', 'required', 'error', 'problem', 'issue'];
            if (importantKeywords.some(keyword => msg.content.toLowerCase().includes(keyword))) score += 25;
        ...
        const selectedMessages = Array.from(new Set([...importantMessages, ...recentMessages]))
        ...
        return {
            ...
            contextTrimDetails: {
                ...
                preservedContext: selectedMessages.map(m => `${m.role} (score: ${m.importance?.toFixed(0)})`)
            }
        };
    }
} 